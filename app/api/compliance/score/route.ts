...
export async function GET(req: Request): Promise<Response> {
  return withRequestContext(req, async (ctx) => {
    ...
    // Early-fail with a structured 503 when the JIT secret isn't configured.
    ...
    try {
      const upstream = await fetch(`${GO_API}/api/compliance/score`, {
        headers: {
      ...
      return ctx.errorJson(
        {
          ...
          message: "No data available. Connect a cloud environment to begin scanning.",
          detail,
        },
        502,
      );
