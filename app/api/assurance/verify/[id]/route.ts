...
// BFF proxy → Go evidence verification (GET /api/assurance/verify/:id).
// Recomputes the per-tenant signature + content hash for one evidence record.
  ...
  return withRequestContext(req, async (ctx) => {
    ...
    try {
      const upstream = await fetch(
        `${GO_API}/api/assurance/verify/${encodeURIComponent(id)}`,
        {
          headers: {
