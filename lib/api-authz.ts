/**
 * SYNCED FROM the private monorepo (frontend/lib/api-authz.ts) — source of truth.
 *
 * Extracted out of middleware.ts so lib/contracts/rbac-authz-contract.test.ts can
 * import it without pulling the server runtime (lib/auth.ts -> lib/env.ts ->
 * "server-only", which does not resolve outside a Next build). Keep this module
 * import-light: `import type` only.
 *
 * Drift between this file and the monorepo is a security defect — the contract
 * test exists to catch middleware becoming looser than the route handlers.
 */
/**
